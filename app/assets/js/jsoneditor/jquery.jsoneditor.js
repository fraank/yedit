...
// Copyright (c) 2013 David Durman, https://github.com/DavidDurman/FlexiJsonEditor
...
(function( $ ) {
    ...
    function addListAppender(item, handler) {
        var btn      = $('<button></button>', { 'class': 'property' });
    ...
    function addListAppenderHash(item, handler) {
        var btn      = $('<button></button>', { 'class': 'property' });
    ...
    function addListAppenderArray(item, handler) {
        var btn      = $('<button></button>', { 'class': 'property' });
    ...
    function construct(opt, json, root, path) {
        ...
        if (isObject(json) || isArray(json)) {
            ...
            appender.append( addListAppender(root, function () {
                addNewValue(json);
                construct(opt, json, root, path);
                opt.onchange(parse(stringify(opt.original)));
            }));
            appender.append( addListAppenderHash(root, function () {
                addNewHash(json);
                construct(opt, json, root, path);
                updateParents(json, opt);
                opt.onchange(parse(stringify(opt.original)));
            }));
            appender.append( addListAppenderArray(root, function () {
                addNewArray(json);
                construct(opt, json, root, path);
                updateParents(json, opt);
                opt.onchange(parse(stringify(opt.original)));
